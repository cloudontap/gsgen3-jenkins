...
for CURRENT_SLICE in ${SLICE_LIST}; do
    ...
    ${GSGEN_DIR}/createApplicationTemplate.sh -c ${PRODUCT_CONFIG_COMMIT} -s ${CURRENT_SLICE}
    RESULT=$?
    if [[ ${RESULT} -ne 0 ]]; then
    ...
    if [[ "${MODE}" == "stopstart" ]]; then 
        ${GSGEN_DIR}/createStack.sh -t application -s ${CURRENT_SLICE}
        RESULT=$?
    fi
    if [[ "${MODE}" == "update"    ]]; then 
        ${GSGEN_DIR}/updateStack.sh -t application -s ${CURRENT_SLICE}
        RESULT=$?
    fi
